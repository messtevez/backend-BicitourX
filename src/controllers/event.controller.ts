...
const createEvent = async (req: Request, res: Response): Promise<Response> => {
    const { title, date, hour, location, cost, distance, capacity, category, attendees, eventImg, descripcion } = req.body;
    try {
...
const updateEventById = async (req: Request, res: Response): Promise<Response> => {
    const id = req.params.id;
    const { title, date, hour, location, cost, distance, capacity, category, descripcion } = req.body;
    try {
...
const deleteEventById = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        ...
        if (event) {
            ...
            return res.status(200).json({
                ok: true,
                msg: `El evento ${event.title} ha sido eliminado.`,
            });
        }
...
const requestToJoinEvent = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        ...
        const adminSubject = `Solicitud de registro al evento: ${valEvent.title}`;
        const adminText = `El usuario ${user.nombre} (${user.email}) ha solicitado unirse al evento "${valEvent.title}". ID de usuario: ${idUser}.`;
...
const addAttendeeByAdmin = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        ...
        if (!user) {
            logger.warn(`User not found with ID: ${emailUser}`);
            return res.status(400).json({
                ok: false,
        ...
        const exists = valEvent.attendees.includes(emailUser);
        if (exists) {
            logger.info(`Este ${emailUser} ya esta inscrito en este evento.`);
            return res.status(400).json({
                ok: false,
                msg: `Este ${emailUser} ya esta inscrito en este evento.`,
            });
        }
        ...
        if (valEvent.capacity <= valEvent.attendees.length) {
            ...
            return res.status(400).json({
                ok: false,
                msg: "Evento lleno, no se pueden agregar mas usuarios. ",
            });
        }
        console.log(emailUser)
        valEvent.attendees.push(user._id);
        await valEvent.save();

        const adminEmail = 'admin@example.com';
        const adminSubject = `Asistente añadido al evento: ${valEvent.title}`;
        const adminText = `El usuario ${user.nombre} (${user.email}) ha sido añadido al evento "${valEvent.title}". Capacidad restante: ${valEvent.capacity - valEvent.attendees.length}.`;
        await sendEmail(adminEmail, adminSubject, adminText);
        logger.info(`Admin notified of new attendee for event with ID: ${idEvent}`);

        const userSubject = `Registro confirmado para el evento: ${valEvent.title}`;
        const userText = `Hola ${user.nombre},\n\nTe has registrado exitosamente para el evento "${valEvent.title}". ¡Nos vemos ahí!`;
...
const getEventsByUserId = async (req: Request, res: Response): Promise<Response> => {
    const userId = req.params.userId;

    try {
        const eventos = await Events.find({ attendees: userId });

        if (eventos.length === 0) {
            return res.status(404).json({
                ok: false,
                msg: 'No se encontraron eventos para este usuario'
            });
        }
        ...
        return res.status(500).json({
            ok: false,
            msg: 'Error al obtener los eventos del usuario',
            error
        });
    }
};
